...
import { FaWind, FaCloud, FaSun, FaTint, FaCloudSunRain, FaUmbrella} from 'react-icons/fa';
...
const MoreWeather = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch('https://api.openweathermap.org/data/2.5/weather?q=India&appid=8ed6b75ac5bb4d07f81b77c3a9a65ea5')
  ...
  return (
    <div style={{ backgroundColor: "rgb(5, 8, 22)" }} className=" text-white min-h-screen flex flex-col justify-center items-center p-6 space-y-8">
      <div className="flex  flex-wrap justify-center gap-6 mb-8">
        ...
        />
                {/* Suggestions Section */}
       <div className="bg-gray-900 p-6 ml-10 mr-10 flex items-center rounded-lg  w-full max-w-lg shadow-lg">
       
        <div className="flex-1 ml-24 space-y-4">
        <h2 className="text-xl  flex-1  font-bold mb-4">Suggestions for Today</h2>
          {suggestions &&
...
const WeatherCard = ({ title, value, description, icon, bgColor }) => (
  <div className={`p-6 rounded-lg w-72 shadow-lg transform transition-transform duration-300 ease-in-out hover:scale-105 flex flex-col items-center border-4 border-transparent ${bgColor} bg-opacity-80 hover:border-${bgColor.replace("bg-", "")}`}>
